...
const tryCreateDelhiveryShipment = async (order) => {
  try {
    ...
    const payload = {
      pickup_location: { name: pickup }, // Some API versions expect object, some expect string. We'll try string first as it's more common.
      shipments: [
        {
    ...
    const success = data?.success || pkg?.status === "Success" || !!wbFinal;
    ...
    const errorMsg = pkg?.remarks?.[0] || data?.message || data?.error || "Delhivery API error";
