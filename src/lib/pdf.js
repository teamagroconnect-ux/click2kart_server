...
export const streamInvoicePDF = (res, bill, customer) => {
  ...
  if (companyAddress) doc.fontSize(10).text(companyAddress);
  ...
  bill.items.forEach((it, idx) => {
    doc.text(
      `${idx + 1}. ${it.name} x${it.quantity}  @${it.price}  GST ${it.gst}%  = ${it.lineTotal.toFixed(
        2
      )}`
    );
  });
  ...
  if (bill.discount && bill.discount > 0) {
    doc.text(
      `Discount: -${bill.discount.toFixed(2)}${
        bill.couponCode ? ` (Coupon ${bill.couponCode})` : ""
      }`
    );
